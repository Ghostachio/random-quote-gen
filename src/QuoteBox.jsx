...
const QuoteBox = ({data, random}) => {
  ...
  return (
    <div className='container flex flex-col text-center p-8 m-6 w-auto bg-Grayish-Blue rounded-3xl relative shadow-xl shadow-Dark-Grayish-Blue'>
      ...
      <h1 className=' text-Light-Cyan pb-8'>{data[randomNbr].text}</h1>
      ...
      <div className='flex justify-center text-center relative'>
        <button className='bg-Neon-Green p-3 w-10 rounded-full absolute text-center translate-l-1/2 mt-1 shadow-xl shadow-Neon-Green transform hover:scale-125 duration-700' onClick={random}><img src={dice}/></button>
      </div>
    </div>
  );
  }
